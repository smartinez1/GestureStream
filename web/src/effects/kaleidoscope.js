  ...
  apply(ctx, video, W, H, dt = 1 / 30, box = null, center = null, active = null) {
    ...
    if (box !== null) {
      ...
      if (bw >= 4 && bh >= 4) {
        ...
        x0 = Math.max(0, Math.round(cx - hw));
        const x1 = Math.min(W, Math.round(cx + hw));
        y0 = Math.max(0, Math.round(cy - hh));
    ...
    if (tile !== null) {
      ...
      pat.setTransform(new DOMMatrix().translateSelf(x0 % (tw * 2), y0 % (th * 2)));
      lctx.save();
    ...
    if (tile !== null && full > keep) {
      ...
      for (let i = 0; i <= 8; i++) {
        const t = i / 8;
        const ss = t * t * (3 - 2 * t);
        g.addColorStop(t, `rgba(255,255,255,${ss.toFixed(3)})`);
      }
      mctx.fillStyle = g;
